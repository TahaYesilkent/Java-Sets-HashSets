...
public class StringSet {
    ...
    public static Set<String> findUniqueWords() {
        ...
        // Tüm noktalama işaretlerini temizle (apostrof ve tire dahil)
        String cleanedText = text.replaceAll("[\\p{Punct}]", " ").toLowerCase();

        // Boşluklara göre ayır
        String[] wordsArray = cleanedText.split("\\s+");

        // Unique ve alfabetik sıralı kelimeler
        Set<String> uniqueWords = new TreeSet<>(Arrays.asList(wordsArray));

        // Boş stringleri temizle
        uniqueWords.remove("");

        System.out.println("--- Unique Kelime Analizi ---");
        System.out.println("Toplam Unique Kelime Sayısı: " + uniqueWords.size());
